/*!
 * CANONICAL source of the Plan Match Supplement TCPA consent language.
 *
 * Rendered by src/components/Application.tsx and mirrored byte-for-byte
 * into api/_lib/tcpa-consent.ts (see the header there for why the copy
 * exists). Edit THIS file. `npm run build` runs
 * scripts/check-tcpa-mirror.mjs, which fails the build if the two drift.
 *
