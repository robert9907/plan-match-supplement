...
if (/^(Edit|Write|MultiEdit|NotebookEdit)$/.test(tool)) {
  ...
  if (!rel.startsWith('..') && isProtected(rel)) {
    ask(`${rel} is a protected file — gate/validator logic or regulated compliance text. Changing it changes what "passing" means, or what the beneficiary is told. Approve only if you asked for this change.`);
  }
  process.exit(0);
}
