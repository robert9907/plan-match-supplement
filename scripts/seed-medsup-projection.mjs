...
// Retired for data quality, not supersession. CMS files no equivalent series,
// so nothing can check these against the source of truth — and each is already
// known to be wrong. Keeping them means one line on an otherwise all-CMS chart
// comes from somewhere else, which makes the disclosure's source statement
// false. Retiring is the honest option; re-add a carrier here only with a
// filing you can point at.
...
function supersede() {
  ...
  const unverifiable = RETIRED_UNVERIFIABLE[state] ?? {};
  const retire = [...Object.entries(map).filter(([, v]) => v).map(([k]) => k), ...Object.keys(unverifiable)];
  ...
  for (const [k, why] of Object.entries(unverifiable)) console.log(`--   RETIRE  ${k}  ->  no CMS counterpart; ${why}`);
  console.log('');
  console.log('begin;');
  console.log('');
